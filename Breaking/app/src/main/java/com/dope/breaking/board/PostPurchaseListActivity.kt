...
class PostPurchaseListActivity : AppCompatActivity() {
    ...
    private var postId = -1 // 구매 목록을 가져올 게시물 id
    private lateinit var adapterPurchaseList: FollowAdapter // 구매 리스트 어댑터 (팔로잉 어댑터 재사용)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapterPurchaseList = FollowAdapter(this, purchaseList, false, -1) // 초기 어댑터 지정

        /*
           최초 구매자 리스트 요청
         */
        processGetPurchaseList(
            token,
            postId.toLong(),
            0, {
                ...
                recyclerView.adapter = adapterPurchaseList // 리사이클러 뷰에 어댑터 설정
                    ...
                    override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                        ...
                        if (lastIndex == recyclerView.adapter!!.itemCount - 1 && newState == 2 && !isObtainedAll && !isLoading) {
                            processGetPurchaseList(
                                token,
                                postId.toLong(),
                                purchaseList[lastIndex]!!.cursorId, { // 마지막 인덱스
                                    adapterPurchaseList.addItem(null) // 로딩 아이템 추가
                                    isLoading = true // 로딩 상태 on
                                },
                                { it2 ->
                                    if (it2.size < ValueUtil.FOLLOW_SIZE) { // 정량으로 가져오는 개수보다 적다면
                                        adapterPurchaseList.removeLast() // 먼저 로딩 아이템 제거
                                        if (it2.isNotEmpty()) { // 리스트가 비어있지 않다면
                                            adapterPurchaseList.addItems(it2) // 받아온 리스트 추가
                                        }
    ...
    /**
     * @description - 게시물 구매 리스트 요청 함수를 호출하는 메소드
     * @param - token(String) : JWT 토큰
     * @param - postId(Long) : 구매 리스트를 요청할 게시물 id
    ...
    private fun processGetPurchaseList(
        ...
        CoroutineScope(Dispatchers.Main).launch {
            ...
            try {
                val response = postManager.startGetPostPurchase(
                    token,
                    postId,
                    lastUserId,
                    ValueUtil.PURCHASE_SIZE,
                )
    ...
    private fun extractMyselfItem() {
        var i = 0
        for (likeData in adapterPurchaseList.data) {
            if (likeData!!.userId == ResponseExistLogin.baseUserInfo?.userId && i != 0) {
                adapterPurchaseList.removeItem(likeData) // 현재 데이터 제거
                adapterPurchaseList.addItemIndex(0, likeData) // 첫번째 인덱스에 추가
                break
            }
            i++
        }
    ...
    private fun handleEmptyList() {
        ...
        setToolbar() // 툴바 설정
        findViewById<TextView>(R.id.alert_text).text = "구매자 목록이 없습니다."
    }
    ...
    private fun setToolbar() {
        ...
        toolbar.title = "구매자 목록" // 툴바 타이틀 설정
        // 뒤로가기 버튼 설정
