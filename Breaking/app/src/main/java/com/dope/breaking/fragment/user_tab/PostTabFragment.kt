...
class PostTabFragment : Fragment() {
    ...
    private val requestOption = "write" // 현재 Fragment 의 옵션 - 작성한 제보글
    ...
    override fun onCreateView(
        ...
        val userId = arguments?.getLong("userId") // 유저페이지로부터 받아온 유저 id
        val token =
            ValueUtil.JWT_REQUEST_PREFIX + JwtTokenUtil(requireContext()).getAccessTokenFromLocal()

        // 리사이클러 뷰 divider
        binding.rcvUserWritten.addItemDecoration(
        ...
        processGetUserFeed(userId!!, 0, ValueUtil.FILTER_SELL_OPTIONS[0], token, {
            ...
            /*
                스크롤 이벤트
             */
            binding.rcvUserWritten.addOnScrollListener(object : RecyclerView.OnScrollListener() {
    ...
    private fun showSkeletonView() {
        binding.rcvUserWritten.visibility = View.GONE
    ...
    private fun dismissSkeletonView() {
        ...
        binding.rcvUserWritten.visibility = View.VISIBLE
    }
